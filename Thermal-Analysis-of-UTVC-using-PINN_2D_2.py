...
# ---------------------------------------
# Adjusted Characteristic scales
L_star = L        # Length scale (m)
U_star = 0.05    # Reduced Velocity scale (m/s)
T_star = 40       # Temperature difference scale (K), from 30°C to 70°C
C_star = 0.1      # Concentration scale (kg/kg)
...
Nu = 100.0  # Adjusted value to represent realistic convection
...
num_hidden_layers = 6  # Increased layers for better capacity
num_neurons_per_layer = 50  # Increased neurons per layer
activation_function = tf.nn.tanh

# Training parameters
learning_rate = 1e-3   # Further reduced learning rate
epochs = 10000         # Increased number of epochs
batch_size = 1024      # Batch size remains the same
...
N_col = 20000  # Increased number for better PDE enforcement
...
N_evap = 3000  # Increased to better enforce boundary conditions
...
N_init = 8000  # Increased to better enforce initial conditions
...
# ---------------------------------------
# 7. Model Definition with Corrected Architecture
# ---------------------------------------
class PINN(tf.keras.Model):
    def __init__(self):
        super(PINN, self).__init__()
        self.hidden_layers = []
        # Removed InputLayer as it's not needed in subclassed models
        for _ in range(num_hidden_layers):
            self.hidden_layers.append(tf.keras.layers.Dense(
                num_neurons_per_layer, activation=activation_function,
                kernel_initializer='glorot_normal'))
        
        # Output layers with appropriate activation functions and float32 dtype
        self.u_output = tf.keras.layers.Dense(1, activation=None, dtype='float32')
        self.v_output = tf.keras.layers.Dense(1, activation=None, dtype='float32')
        self.p_output = tf.keras.layers.Dense(1, activation=None, dtype='float32')
        self.T_output = tf.keras.layers.Dense(1, activation=None, dtype='float32')  # Linear activation
        self.C_output = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid, dtype='float32')  # To ensure C is between 0 and 1
    ...
    def call(self, X):
        ...
        T = self.T_output(Z)
        C = self.C_output(Z)
        return tf.concat([u, v, p, T, C], axis=1)
...
# 8.1. PDE Residual Loss with Scaling Factors
def compute_pde_loss(model, X):
    ...
    # Scaled Navier-Stokes equations residuals (dimensionless form)
    momentum_u = (Re * (u_t + u * u_x + v * u_y) + p_x - (u_xx + u_yy)) / Re
    momentum_v = (Re * (v_t + u * v_x + v * v_y) + p_y - (v_xx + v_yy)) / Re
    
    # Scaled Energy equation residual
    energy = (Pe_T * (T_t + u * T_x + v * T_y) - (T_xx + T_yy)) / Pe_T
    
    # Scaled Surfactant transport equation residual
    surfactant = (Pe_C * (C_t + u * C_x + v * C_y) - (C_xx + C_yy)) / Pe_C
...
# 8.2. Boundary Conditions Loss with Corrected Implementation
def compute_boundary_loss(model, X_evap_cpu, X_evap_gpu, X_cond_left, X_cond_right, X_top, X_bottom):
    ...
    mse_cond_left = tf.reduce_mean(tf.square(-T_n_cond_left + Nu * (T_cond_left - 0)))  # Enforcing T -> 0 (ambient)
    ...
    mse_cond_right = tf.reduce_mean(tf.square(T_n_cond_right + Nu * (T_cond_right - 0)))
...
# 8.4. Combined Loss Function with Adjusted Weighting
def loss_function(model, X_col, X_evap_cpu, X_evap_gpu, X_cond_left, X_cond_right,
    ...
    total_loss = loss_pde + 5.0 * loss_bc + 0.05 * loss_ic
...
def plot_distributions(model, t_physical):
    ...
    contour = plt.contourf(X_dim, Y_dim, T_pred_dim, levels=50, cmap='jet')
    ...
    plt.quiver(X_dim[skip], Y_dim[skip], u_pred_dim[skip], v_pred_dim[skip], scale=50)
    plt.xlabel('x (mm)')
    plt.ylabel('y (mm)')
    ...
    plt.figure(figsize=(8, 6))
    contour = plt.contourf(X_dim, Y_dim, p_pred_dim, levels=50, cmap='viridis')
    ...
    plt.figure(figsize=(8, 6))
    contour = plt.contourf(X_dim, Y_dim, C_pred_dim, levels=50, cmap='plasma')
    plt.colorbar(contour, label='Concentration (kg/kg)')
    plt.xlabel('x (mm)')
    plt.ylabel('y (mm)')
